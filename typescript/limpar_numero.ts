...
const limpar_numero = (numero:numero, n:number=-1):string_numerica => {
    /* "limpar_numero" aceita 3 tipos de dados: números, strings ou
      objetos iteráveis do tipo array, e retorna uma string contendo
      apenas caracteres numéricos (de 0 a 9, ou um inteiro convertido
      em string).
        Primeiramente, é necessário obter uma string intermediária,
      portanto, caso receba um array, a função irá iterar na primeira
      camada de itens, e, caso sejam do tipo number ou string, serão
      covertidos para string, caso forem números, e então concatenados
      a uma variável. Caso "limpar_numero" receba um número, o mesmo
      será convertido em string.
        Uma vez obtida uma string a partir do argumento recebido, os
      caracteres da mesma serão checados se são números de 0 a 9: em
      caso positivo, serão concatenados a uma variável, que será
      retornada ao final da execução.
    */
    if (n == -1)
        n = numero.toString().length;
    ...
    return numero_limpo.padStart(n, '0')
}
