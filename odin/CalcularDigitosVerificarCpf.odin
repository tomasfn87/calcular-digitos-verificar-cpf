...
main :: proc() {
    fmt.println(`reterNumeros("a1", 2) =`, reterNumeros("a1", 2))     // "01"
    fmt.println(`reterNumeros("b23", 2) =`, reterNumeros("b23", 2))   // "23"
    fmt.println(`reterNumeros("c456", 2) =`, reterNumeros("c456", 2)) // "45"
    fmt.println(`calcularDigitos("123") =`, calcularDigitos("123"))   // [6, 0]
    fmt.println(`verificar("12359") =`, verificar("12359"))           // false
    fmt.println(`verificar("12360") =`, verificar("12360"))}          // true
...
calcularDigitos :: proc(cpf: string) -> [dynamic]u8 {
    if len(reterNumeros(cpf, 1)) < 1 {
        return [dynamic]u8{}}
    dvs := [dynamic]u8{ ---, --- }
    multiplicador, resto, soma : u8 = 10, 0, 0
